...
|---|---|---|---|
| `group` | String | `group` → `agentID` | agent 命名空间。**有 `agent_status` 时同时充当 agent_id**。缺失时 agentID = `"default"`。 |
| `task_id` | String | `taskID` | 任务唯一 ID（如 `auth-migration-0420`）。同 `agent_id + task_id` 的多条 push 通过 `AgentTask.aggregateKey` 聚合，原地更新 task、追加 step。 |
...
- ❌ `agent_id` — **没有这个字段**；用 `group` 代替（设计上 `group` 同时充当 agent_id 与 task 命名空间）
