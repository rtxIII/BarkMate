...
<title>BarkMate Privacy Policy / 隐私政策</title>
<style>
...
<h1>BarkMate Privacy Policy</h1>

<p>This policy describes how the BarkMate iOS application ("the App") and the BarkMate
...
<h2>1. Who we are</h2>
<p>BarkMate is an open-source project. We do not operate analytics, advertising,
...
keys, optional AES key (Keychain only), AgentTask / AgentStep / Memo records, cached push
images. Stored inside the App Group container <code>group.com.barkmate.shared</code>.</p>
...
<p>Name, email, phone, contacts, calendar, photos, location, IDFA, device fingerprint,
browsing history, microphone, camera, health, biometric data — none. The App never asks
for these permissions.</p>
...
<h2>5. Sharing</h2>
<p>We do not sell, rent, or share your data. The only entities that ever see BarkMate
...
<h2>7. Children</h2>
<p>BarkMate is not directed to children under 13 and we do not knowingly collect data
from them.</p>
...
<p>本政策说明 BarkMate iOS 应用("应用")与 BarkMate 自部署推送中继("服务器",运行于
<code>barkmate.we2.xyz</code>)如何处理您的数据。两者均为开源。</p>

<h2>1. 我们是谁</h2>
<p>BarkMate 是开源项目。我们不运营任何分析、广告、归因或崩溃上报基础设施。应用不包含
任何第三方 SDK。</p>

<h2>2. 我们处理哪些数据</h2>
<p><strong>在您的设备上</strong>:APNs device token、已配置的服务器地址 + API key、
可选 AES 密钥(仅 Keychain)、AgentTask / AgentStep / Memo 记录、推送图片缓存。位于
App Group 容器 <code>group.com.barkmate.shared</code> 内。</p>
...
<h2>3. 我们不收集的内容</h2>
<p>姓名、邮箱、电话、通讯录、日历、照片、位置、IDFA、设备指纹、浏览历史、麦克风、
...
<h2>5. 共享</h2>
<p>我们不售卖、出租或共享您的数据。唯一会接触到 BarkMate 流量的第三方:
...
<h2>7. 儿童</h2>
<p>BarkMate 不面向 13 岁以下儿童,我们也不会有意收集其数据。</p>
...
function renderText(): string {
  return [
    ...
    ``,
    `2. Data we process`,
    `   - On device: APNs token, server addresses + keys, optional AES key (Keychain), AgentTask/AgentStep/Memo, cached push images. App Group: group.com.barkmate.shared.`,
    ...
    ``,
    `3. We do NOT collect`,
    `   name, email, phone, contacts, calendar, photos, location, IDFA, device fingerprint, browsing history, microphone, camera, health, biometric data.`,
    ``,
    `4. Encryption`,
    ...
    `2. 数据处理:`,
    `   - 设备本地:APNs token、服务器配置、可选 AES key (Keychain)、AgentTask/AgentStep/Memo、推送图片缓存。位于 App Group group.com.barkmate.shared。`,
    `   - 服务器 KV:仅 device:<key> -> APNs token。不记录推送内容,不持久化 payload。`,
    `   - 传输:HTTPS 至(a) 您配置的 Bark 服务器、(b) 推送内嵌的图片 URL、(c) Apple APNs。`,
    `3. 不收集:姓名、邮箱、电话、通讯录、日历、照片、位置、IDFA、指纹、浏览历史、麦克风、相机、健康或生物特征。`,
