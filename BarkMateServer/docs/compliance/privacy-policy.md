---
title: BarkMate Privacy Policy / 隐私政策
version: 1.0
...
This policy describes how the BarkMate iOS application ("the App") and the
BarkMate self-hosted push relay ("the Server", deployed at
...
BarkMate is an open-source project. The App is distributed through the
...
Contact: open an issue at `https://github.com/<owner>/BarkMemo/issues`.
...
2. **Image URLs embedded inside a push payload** — when the sender
   ...
   by whoever sent the push, not by BarkMate.
...
contacts, calendar, photos, location, advertising identifier (IDFA),
device fingerprint, browsing history, microphone, camera, health, or
biometric data. The App never asks for these permissions.
...
ever see any BarkMate-related traffic are:
...
BarkMate is not directed to children under 13 and we do not knowingly
...
本政策说明 BarkMate iOS 应用("应用")与 BarkMate 自部署推送中继
...
BarkMate 是开源项目。应用通过 App Store 分发,服务器由维护者运行在
`barkmate.we2.xyz`,任何用户也可以自行部署。

我们不运营任何分析、广告、归因或崩溃上报基础设施。应用不包含任何第三方
SDK。

联系方式: `https://github.com/<owner>/BarkMemo/issues` 提 issue。
...
2. **推送 payload 内嵌的图片 URL** — 发推送时若带 `image=` 字段,
   NSE 会下载图片以渲染富通知。图片的宿主由发送方决定,而不是 BarkMate。
...
我们不收集、传输或存储: 姓名、邮箱、电话、通讯录、日历、照片、位置、
广告标识(IDFA)、设备指纹、浏览历史、麦克风、相机、健康或生物特征数据。
应用从不申请这些权限。
...
我们**不**售卖、出租或共享您的数据。唯一会接触到 BarkMate 相关流量的
第三方:
...
BarkMate 不面向 13 岁以下儿童设计,我们也不会有意收集他们的数据。
